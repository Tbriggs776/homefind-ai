 ...
 * Closed, Pending, Coming Soon, Cancelled, and Sold listings to discover
 * what fields are available for each status. Does NOT store any data.
 *
 ...
 * Admin-only gated.
 *
 * Invoke with an empty body from an admin session:
 *   POST /functions/v1/probeSparkClosedListings
...
/**
 * Anonymize a record before returning it — strip fields that might contain
 * listing-agent-private notes or buyer contact info. MLS address/price/specs
 * data stays (licensed agents are entitled to this through Spark).
 */
...
serve(async (req) => {
  ...
  try {
    ...
    const unionOfFields = new Set<string>();
    ...
    for (const probe of PROBE_STATUSES) {
      try {
        ...
        listings.forEach((listing: any) => {
          ...
          // Capture top-level non-StandardFields keys like ListingKey, Id
          Object.keys(listing).forEach(k => {
            if (k !== 'StandardFields') {
              fieldsInThisStatus.add(`_root.${k}`);
              unionOfFields.add(`_root.${k}`);
            }
          });
        });
    ...
    results.union_of_all_fields = Array.from(unionOfFields).sort();
